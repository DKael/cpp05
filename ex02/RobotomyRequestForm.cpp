...
RobotomyRequestForm::RobotomyRequestForm()
	: AForm("RobotomyRequestForm" + intToString(RobotomyRequestForm::count), 72, 45), target("") {
	RobotomyRequestForm::count++;
	srand(static_cast<unsigned int>(time(NULL)));
}

RobotomyRequestForm::RobotomyRequestForm(const std::string& _target)
	: AForm("RobotomyRequestForm" + intToString(RobotomyRequestForm::count), 72, 45), target(_target) {
	RobotomyRequestForm::count++;
	srand(static_cast<unsigned int>(time(NULL)));
}

RobotomyRequestForm::RobotomyRequestForm(const RobotomyRequestForm& origin)
	: AForm("RobotomyRequestForm" + intToString(RobotomyRequestForm::count), 72, 45), target(origin.target) {
	RobotomyRequestForm::count++;
	srand(static_cast<unsigned int>(time(NULL)));
}
